...
Java_pl_droidsonroids_gif_GifInfoHandle_glTexImage2D(JNIEnv *__unused unused,
    ...
    glTexImage2D((GLenum) target, level, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 pixels);
    pthread_mutex_unlock(&descriptor->renderMutex);
}

__unused JNIEXPORT void JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_glTexSubImage2D(JNIEnv *__unused env,
                                                        jclass __unused handleClass, jlong gifInfo,
                                                        jint target, jint level) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->frameBufferDescriptor == NULL) {
...
static void *slurp(void *pVoidInfo) {
    ...
    while (true) {
        ...
        const long long invalidationDelayMillis = calculateInvalidationDelay(info, renderStartTime,
                                                                             frameDuration);
        const int pollResult = TEMP_FAILURE_RETRY(
                poll(&texImageDescriptor->eventPollFd, 1, (int) invalidationDelayMillis));
        if (pollResult < 0) {
...
__unused JNIEXPORT void JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_initTexImageDescriptor(JNIEnv *env,
...
__unused JNIEXPORT void JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_startDecoderThread(JNIEnv *env, jclass __unused handleClass,
                                                           jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL) {
        return;
...
__unused JNIEXPORT void JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_stopDecoderThread(JNIEnv *env, jclass __unused handleClass,
                                                          jlong gifInfo) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL || info->frameBufferDescriptor == NULL) {
        return;
...
__unused JNIEXPORT void JNICALL
Java_pl_droidsonroids_gif_GifInfoHandle_seekToFrameGL(__unused JNIEnv *env,
                                                      jclass __unused handleClass, jlong gifInfo,
                                                      jint desiredIndex) {
    GifInfo *info = (GifInfo *) (intptr_t) gifInfo;
    if (info == NULL) {
...
Java_com_wenchao_gifloader_GifHandle_openFile(JNIEnv *env, jobject instance, jstring path_) {
    return openFile(env, instance, path_);
}

JNIEXPORT jlong JNICALL
Java_com_wenchao_gifloader_GifHandle_renderFrame(JNIEnv *env, jobject instance, jlong gifInfo,
                                              jobject frameBuffer) {

    return renderFrame(env, instance, gifInfo, frameBuffer);

}


JNIEXPORT jint JNICALL
Java_com_wenchao_gifloader_GifHandle_getWidth(JNIEnv *env, jobject instance, jlong gifInfo) {
...
JNIEXPORT jint JNICALL
Java_com_wenchao_gifloader_GifHandle_getHeight(JNIEnv *env, jobject instance, jlong gifInfo) {
